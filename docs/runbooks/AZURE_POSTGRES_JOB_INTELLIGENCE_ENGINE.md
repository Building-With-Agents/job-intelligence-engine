...
3. The script writes one JSON file per table into `scripts/pg-seed-data/fixtures/` (reference) and `scripts/pg-seed-data/agent-fixtures/` (pipeline) and updates each `metadata.json` with row counts. PII tables are skipped; see the script for the full exclusion list.
4. Commit and push the updated fixture directories (and optionally `schema.sql` if you changed schema) so others can use them.
