...
- **Tests**: `tests/test_llm_summary.py` — 8 cases (LLM success, exceptions, empty content, sector/skill keys, mixed maps, template numeric fidelity, never-raises). `tests/test_analytics_events.py` — 7 cases on `build_analytics_refreshed_event` (payload shape, counts, ISO `refreshed_at`, envelope ids). `tests/test_analytics_agent.py` updated for the new `AnalyticsRefreshed` payload.
