...
- `streamlit run dashboard/app.py` with PostgreSQL populated from prior ingestion/normalization runs (when available).
...
- Unit tests: `tests/test_dashboard_observability.py` (metric helpers), `tests/test_streamlit_app.py` (including `_db_available` / read-only URL cases).
- **Regression:** Full suite `pytest tests/` — counts vary with environment (**193+** when DB available; **~235** in a fully green local run). **Caveat:** tests that run **Crawl4AI / Playwright** fail if browsers are not installed (`playwright install`); those failures are environmental, not Streamlit regressions.
...
- Keep `pytest tests/ -v` green on CI; merge only after the full suite passes in your pipeline configuration (or explicitly skip/mock Crawl4AI where appropriate).
...
- **Streamlit reruns** when the tab reconnects; widget selections (e.g. which ingestion run) are **not** persisted in `session_state` on all pages — the UI can “reset” while **Postgres data is unchanged**. Recover runs via **Pipeline Run Summary** (newest run usually first), **`data/output/pipeline_run.json`**, or SQL on `job_ingestion_runs`.

## Data / Evidence

- Code: `dashboard/app.py` (repo-root `sys.path` bootstrap), `readonly_engine.py`, `observability_queries.py`, `observability_metrics.py`, `pages_observability.py`, updates to `streamlit_app.py` (e.g. `width="stretch"` for Streamlit API deprecation).
- Tests: `tests/test_dashboard_observability.py` (9 tests), `tests/test_streamlit_app.py`; full `tests/` as run in your environment.
- Dependency: `plotly>=5.18` in `requirements.txt`.

## Final verification performed (2026-04-01)

- **Automated:** `pytest tests/test_dashboard_observability.py tests/test_streamlit_app.py` → **30 passed**; full `pytest tests/` → **193 passed** (5 unrelated SQLAlchemy deprecation warnings). `ruff check dashboard/` → clean. Listed dashboard modules present on disk; `python -c "from agents.dashboard.app import main"` succeeds (Streamlit “no runtime” cache warnings on import are expected outside `streamlit run`).
- **Manual UI:** `streamlit run dashboard/app.py` — **Ingestion Overview** and **Normalization Quality** render without errors; empty ingestion tables show the intended info callouts; normalization/quarantine metrics reflect whatever rows exist in `normalized_jobs` / `normalization_quarantine` (pages can differ if staging vs downstream tables are out of sync in dev — not a wiring bug).
...
- **Tests:** `TestRawRowIngestionSucceeded` in `tests/test_streamlit_app.py`.
...
- **Tests:** `tests/test_batch_insights_queries.py`, `tests/test_dashboard_list_window.py`.
...
# DB migrate
.venv/bin/python scripts/db_check.py migrate
```
