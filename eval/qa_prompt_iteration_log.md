...
| `week11-final` | 2026-05-04 | Bryan | v2 automated scorer | **Week 11 final demo run.** Full 90q corpus, local DB (missing `label_embedding` → 26 infra errors on geographic/role_evolution/workflow). Scored pool: `intent_accuracy` 0.797, `evidence_citation` 0.546, `confidence_self_consistency` 0.940, `latency_sla` 1.000, `answerability` 0.533 (+0.073 vs dev-verify). p50 latency 2.0s, p95 11.0s. 100% evidence citation rate on scored items. Consistent with dev-verify within noise band. | **0.680** geo | **0.533** (n=30) | — | [`eval/runs/findingsv2.31.md`](runs/findingsv2.31.md) |
