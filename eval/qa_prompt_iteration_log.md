...
| 2026-06-02 | PR #378 review response | **c6 — c5 carve-out tightening** (PR review blockers #1 and #2): (1) ROLE_EVOLUTION CARVE-OUT in `analytics/query_engine/intent.py` `_SYSTEM_PROMPT` tightened with an explicit PREREQUISITE gate (carve-out only applies when question contains explicit AI framing); sub-condition (a) now requires AI-assist tool to be explicitly named in question text; sub-condition (b) now requires explicit AI-literacy / AI-familiarity language rather than a generic "bar" or "seniority level". (2) `evidence_overlap` regression documented in `eval/runs/findings-disruption-narrow-fix.md` §7 as a denominator sampling artifact (n=44→53; total sum unchanged at ~30.98→29.99; newly-correctly-classified items enter the scored population with lower scores). (3) Missing c2–c4 intermediate artifact explanation added to §7. Smoke run 6/6 pass (gq-017, gq-021, gq-026, gq-027, gq-029, gq-030). Anti-regression: gq-033, gq-036, gq-039 → role_evolution ✓; gq-001, gq-002, gq-005, gq-008, gq-010 → disruption ✓ (8/8). |
