"""On-demand trigger computations with cache + SQL guardrails."""
...
from analytics.api.schemas import TriggerEnvelope
...
def run_cohort_gap_analysis(
    ...
    *,
    ...
    vr = validate_sql(sql)
    audit.log_sql_validation_to_llm_audit(sql_text=sql, is_valid=vr.ok, reason=vr.reason)
    if not vr.ok:
