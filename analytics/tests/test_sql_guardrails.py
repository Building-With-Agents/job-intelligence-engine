...
def test_validate_accepts_simple_select() -> None:
    ok, reason, norm = validate_ask_the_data_sql("SELECT job_posting_id FROM dbo.job_postings")
...
def test_validate_rejects_drop() -> None:
    ok, reason, norm = validate_ask_the_data_sql("DROP TABLE dbo.job_postings")
    assert not ok
    assert norm is None


def test_validate_rejects_stacked_statements() -> None:
    ok, reason, norm = validate_ask_the_data_sql("SELECT 1 FROM dbo.job_postings; DELETE FROM dbo.job_postings")
    assert not ok
...
def test_validate_rejects_disallowed_table() -> None:
    ok, reason, norm = validate_ask_the_data_sql("SELECT * FROM dbo.pg_stat_activity")
    assert not ok
...
def test_validate_rejects_insert() -> None:
    ok, reason, norm = validate_ask_the_data_sql("INSERT INTO dbo.job_postings (job_title) VALUES ('x')")
    assert not ok
    assert norm is None


def test_validate_rejects_comment_hiding_delete() -> None:
    ok, reason, norm = validate_ask_the_data_sql("SELECT 1 FROM dbo.job_postings WHERE 1=1/**/DELETE/**/AND 1=1")
    assert not ok


def test_validate_clamps_high_limit() -> None:
    ok, _, norm = validate_ask_the_data_sql("SELECT * FROM job_postings LIMIT 500")
...
def test_validate_rejects_too_long() -> None:
    ok, reason, _ = validate_ask_the_data_sql("SELECT 1 FROM job_postings " + ("x" * 30_000))
    assert not ok
